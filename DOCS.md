...
*   `main.go` — собирает приложение Fiber, отдаёт фронтенд с диска из
    `./web-ui/wasm`, поднимает basic auth и слушает `WEB_UI_PORT`.
...
│   ├── 05-polling.spec.js     # Опрос трафика не останавливается
...
make e2e           # поднять чистый тестовый экземпляр и прогнать браузерные тесты Playwright
...
`e2e/shots/`. Спеки делят один бэкенд и опираются на состояние друг друга
(`02` создаёт сервер, к которому `03` добавляет клиента), поэтому выполняются
по порядку файлов в один поток и рассчитывают на экземпляр без настроенных
серверов.
...
`E2E_VOLUME`, `E2E_URL`, `E2E_TIMEOUT`. Прогнать тесты по уже работающему
экземпляру, ничего не пересоздавая, по-прежнему можно напрямую:

```bash
cd e2e && AWG_URL=http://localhost:54845 npx playwright test
```
...
Счётчики интерфейсов и пиров всех серверов одним ответом — то, что фронтенд
опрашивает раз в 5 секунд:

```json
{
  "timestamp": 1757577600,
  "server_traffic": { "<server_id>": { "rx": "1.2 MiB", "tx": "3.4 MiB" } },
  "client_traffic": {
    "<server_id>": {
      "<client_id>": {
