package wgconf

import "amneziawg-web-ui/web-ui/api"

// The I1-I5 signature packets a new client starts from. I1 is a captured
// handshake that makes the tunnel's first packet look like something else on
// the wire; the rest are empty until the operator fills them in.
const (
	DefaultI1 = "<b >"
	DefaultI2 = ""
	DefaultI3 = ""
	DefaultI4 = ""
	DefaultI5 = ""
)

// DefaultISettings is the I1-I5 set a new client starts from. One definition,
// because it is served on its own and again inside every ServerInfo.
func DefaultISettings() api.ISettings {
	return api.ISettings{
		"i1": DefaultI1, "i2": DefaultI2,
		"i3": DefaultI3, "i4": DefaultI4, "i5": DefaultI5,
	}
}

// MergeISettings layers overrides over the defaults, earliest first, so a
// later layer wins. An empty value in a layer does not clear the key: the
// form sends every field, and an untouched one arrives blank.
func MergeISettings(layers ...map[string]string) api.ISettings {
	out := DefaultISettings()
	for _, layer := range layers {
		for k, v := range layer {
			if v != "" {
				out[k] = v
			}
		}
	}
	return out
}
